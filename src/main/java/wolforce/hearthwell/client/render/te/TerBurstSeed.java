...
//	}
	...
	public void render(TeBurstSeed entityIn, float partialTicks, PoseStack matrixStackIn, MultiBufferSource bufferIn, int combinedLightIn,
		...
		BakedModel ibakedmodel = itemRenderer.getModel(itemstack, entityIn.getLevel(), (LivingEntity) null, combinedOverlayIn);
...
//		double d = 0.5 + n / 32.0;
		...
		for (Vector3f vector : new Vector3f[] { Vector3f.XN, Vector3f.YN, Vector3f.ZN }) {
			...
			for (int i = 0; i < n; i++) {
				...
				itemRenderer.render(itemstack, ItemTransforms.TransformType.GROUND, false, matrixStackIn, bufferIn, combinedLightIn, OverlayTexture.NO_OVERLAY,
						ibakedmodel);
